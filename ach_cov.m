function [ach_conv_event] = ach_cov(evt_t,bin_size)
%% used to convolve the event with ACh basis
...
%% generate ACh basis for convolution
...
% Create ACh response
ach_basis = zeros(size(t));
idx = t >= 0;
t_shifted = t(idx);

% Bi-exponential function: fast rise, slow decay
ach_basis(idx) = amp * (1 - exp(-t_shifted / tau_rise)) .* exp(-t_shifted / tau_decay);

%%
cur_event_var = zeros(length(evt_t), 1);
bin_shift = 0;

w = conv(evt_t ,ach_basis');
ach_conv_event = w(1+bin_shift:length(evt_t)+bin_shift);
